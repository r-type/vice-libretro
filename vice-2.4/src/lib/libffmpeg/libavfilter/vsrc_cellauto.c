...
static const AVOption cellauto_options[] = {
#ifdef IDE_COMPILE
	{ "filename", "read initial pattern from file", OFFSET(filename), AV_OPT_TYPE_STRING, {(intptr_t) NULL}, 0, 0, FLAGS },
    { "f", "read initial pattern from file", OFFSET(filename), AV_OPT_TYPE_STRING, {(intptr_t) NULL}, 0, 0, FLAGS },
    { "pattern", "set initial pattern", OFFSET(pattern), AV_OPT_TYPE_STRING, {(intptr_t) NULL}, 0, 0, FLAGS },
    { "p", "set initial pattern", OFFSET(pattern), AV_OPT_TYPE_STRING, {(intptr_t) NULL}, 0, 0, FLAGS },
    ...
    { "rule", "set rule", OFFSET(rule), AV_OPT_TYPE_INT, {110}, 0, 255, FLAGS },
    ...
    { "scroll", "scroll pattern downward", OFFSET(scroll), AV_OPT_TYPE_INT, {1}, 0, 1, FLAGS },
    { "start_full", "start filling the whole video", OFFSET(start_full), AV_OPT_TYPE_INT, {0}, 0, 1, FLAGS },
    { "full", "start filling the whole video", OFFSET(start_full), AV_OPT_TYPE_INT, {1}, 0, 1, FLAGS },
