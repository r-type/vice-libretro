...
static EbmlSyntax matroska_track[] = {
    ...
    { MATROSKA_ID_TRACKOPERATION, EBML_NEST, 0, offsetof(MatroskaTrack, operation), {(intptr_t) matroska_track_operation } },
...
#ifdef IDE_COMPILE
	{ MATROSKA_ID_ATTACHEDFILE, EBML_NEST, sizeof(MatroskaAttachment), offsetof(MatroskaDemuxContext, attachments), {(intptr_t) matroska_attachment } },
#else
...
#ifdef IDE_COMPILE
	{ MATROSKA_ID_BLOCKMORE, EBML_NEST, 0, 0, {(intptr_t) matroska_blockmore} },
#else
...
static EbmlSyntax matroska_blockgroup[] = {
    ...
    { MATROSKA_ID_BLOCKADDITIONS, EBML_NEST, 0, 0, {(intptr_t) matroska_blockadditions} },
...
static int matroska_read_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < chapters_list->nb_elem; i++)
        if (chapters[i].start != AV_NOPTS_VALUE && chapters[i].uid &&
			...
			tmp.den = 1000000000;
			chapters[i].chapter =
                avpriv_new_chapter(s, chapters[i].uid, tmp,
                                ...
                                int64_t discard_padding)
{
    ...
    if (discard_padding) {
	    ...
	    tmp2.num = 1;
		tmp2.den = st->codec->sample_rate;
		AV_WL32(side_data + 4, av_rescale_q(discard_padding, tmp1, tmp2));
#else
...
#ifdef IDE_COMPILE
	if (ts >= matroska->duration * matroska->time_scale) {
		CueDesc tmpx = {-1, -1, -1, -1};
		return tmpx;
	}
#else
...
AVInputFormat ff_matroska_demuxer = {
    ...
    "Matroska / WebM",
    0, "mkv,mk3d,mka,mks",
    0, 0, "audio/webm,audio/x-matroska,video/webm,video/x-matroska",
    0, 0, sizeof(MatroskaDemuxContext),
    matroska_probe,
...
AVInputFormat ff_webm_dash_manifest_demuxer = {
    ...
    "webm_dash_manifest",
    "WebM DASH Manifest",
    0, 0, 0, 0, 0, 0, 0, sizeof(MatroskaDemuxContext),
    0, webm_dash_manifest_read_header,
