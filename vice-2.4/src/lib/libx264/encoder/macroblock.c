...
static ALWAYS_INLINE void x264_mb_encode_chroma_internal( x264_t *h, int b_inter, int i_qp, int chroma422 )
{
 ...
 h->mb.i_cbp_chroma = 0;
    ...
    for( ch = 0; ch < 2; ch++ )
    {
        ...
        for( i8x8 = 0; i8x8 < (chroma422?2:1); i8x8++ )
        {
            if( h->mb.b_trellis )
            {
				int i4x4;
...
static ALWAYS_INLINE int x264_macroblock_probe_skip_internal( x264_t *h, int b_bidir, int plane_count, int chroma )
{
    ...
    for( p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
    {
        ...
        for( i8x8 = 0, i_decimate_mb = 0; i8x8 < 4; i8x8++ )
        {
			...
			nz = h->quantf.quant_4x4x4( dct4x4, h->quant4_mf[quant_cat][i_qp], h->quant4_bias[quant_cat][i_qp] );
    ...
    if( chroma == CHROMA_420 || chroma == CHROMA_422 )
    {
        ...
        for( ch = 0; ch < 2; ch++ )
        {
            ...
            /* calculate dct coeffs */
            for( i8x8 = 0, i_decimate_mb = 0; i8x8 < (chroma422?2:1); i8x8++ )
            {
